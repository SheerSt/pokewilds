    ...
    db 860 ; Dex number.
    db <When it gets down on all fours as if to beg for forgiveness, it's trying to lure opponents in so that it can stab them with its spear-like hair.> ; Dex entry.

    db 12.5 ; Weight in kg.
    db 0.8 ; Height in meters.

    db IMPIDIMP 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).
