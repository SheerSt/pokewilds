    ...
    db 433 ; Dex number.
    db <Each time it hops, it makes a ringing sound. It deafens foes by emitting high-frequency cries.> ; Dex entry.

    db 0.6 ; Weight in kg.
    db 0.2 ; Height in meters.

    db CHINGLING 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).

    db WOODED_LAKE GRAVEYARD ; Spawning biomes (TYPE as the final argument to apply type-based default tiles).
