    ...
    db 41 ; Dex number.
    db <It sleeps in caves during the day. It has no eyes, so to check its surroundings while flying, it emits ultrasonic waves.> ; Dex entry.

    db 7.5 ; Weight in kg.
    db 0.8 ; Height in meters.

    db ZUBAT 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).

    db DEEP_FOREST WOODED_LAKE GRAVEYARD ; Spawning biomes (TYPE as the final argument to apply type-based default tiles).
