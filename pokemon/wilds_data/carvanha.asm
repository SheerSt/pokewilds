    ...
    db 318 ; Dex number.
    db <It won't attack while it's alone�not even if it spots prey. Instead, it waits for other Carvanha to join it, and then the Pok�mon attack as a group.> ; Dex entry.

    db 20.8 ; Weight in kg.
    db 0.8 ; Height in meters.

    db CARVANHA 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).

    db OCEAN ; Spawning biomes (TYPE as the final argument to apply type-based default tiles).
