    ...
    db 74 ; Dex number.
    db <Its round body makes it easy to pick up, but it is hard and heavy. It's dangerous to play with them as if they were snowballs.> ; Dex entry.

    db 20.0 ; Weight in kg.
    db 0.4 ; Height in meters.

    db GEODUDE 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).

    db ROCK_SMASH ROCK_SMASH VOLCANO ; Spawning biomes (TYPE as the final argument to apply type-based default tiles).
