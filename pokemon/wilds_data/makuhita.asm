    ...
    db 296 ; Dex number.
    db <It was originally brought in from another region, but now Makuhita from Alola are more famous.> ; Dex entry.

    db 86.4 ; Weight in kg.
    db 1.0 ; Height in meters.

    db MAKUHITA 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).

    db MOUNTAIN WOODED_LAKE ; Spawning biomes (TYPE as the final argument to apply type-based default tiles).
