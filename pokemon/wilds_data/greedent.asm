    ...
    db 820 ; Dex number.
    db <It stashes berries in its tail�so many berries that they fall out constantly. But this Pok�mon is a bit slow-witted, so it doesn't notice the loss.> ; Dex entry.

    db 6.0 ; Weight in kg.
    db 0.6 ; Height in meters.

    db SKWOVET 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).
