    ...
    db 13 ; Dex number.
    db <Weedle has an extremely acute sense of smell. It is capable of distinguishing its favorite kinds of leaves from those it dislikes just by sniffing with its big red proboscis (nose).> ; Dex entry.

    db 3.2 ; Weight in kg.
    db 0.3 ; Height in meters.

    db WEEDLE 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).

    db FOREST ; Spawning biomes (TYPE as the final argument to apply type-based default tiles).
