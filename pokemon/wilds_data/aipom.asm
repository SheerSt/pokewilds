    ...
    db 190 ; Dex number.
    db <Aipom's tail ends in a hand-like appendage that can be cleverly manipulated. However, because the Pok�mon uses its tail so much, its real hands have become rather clumsy.> ; Dex entry.

    db 11.5 ; Weight in kg.
    db 0.8 ; Height in meters.

    db AIPOM 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).
