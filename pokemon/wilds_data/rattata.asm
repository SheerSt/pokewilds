    ...
    db 19 ; Dex number.
    db <Its incisors continue growing until its death. If its Trainer doesn't offer it a file to gnaw on, it will gnaw on door frames, table legs, and so on.> ; Dex entry.

    db 3.5 ; Weight in kg.
    db 0.3 ; Height in meters.

    db RATTATA 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).

    db SAVANNA SAVANNA PKMNMANSION ; Spawning biomes (TYPE as the final argument to apply type-based default tiles).
