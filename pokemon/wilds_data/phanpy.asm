    ...
    db 231 ; Dex number.
    db <For its nest, Phanpy digs a vertical pit in the ground at the edge of a river. It marks the area around its nest with its trunk to let the others know that the area has been claimed.> ; Dex entry.

    db 33.5 ; Weight in kg.
    db 0.5 ; Height in meters.

    db PHANPY 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).

    db MOUNTAIN SAVANNA WOODED_LAKE ; Spawning biomes (TYPE as the final argument to apply type-based default tiles).
