    ...
    db 115 ; Dex number.
    db <Although it's carrying its baby in a pouch on its belly, Kangaskhan is swift on its feet. It intimidates its opponents with quick jabs.> ; Dex entry.

    db 80.0 ; Weight in kg.
    db 2.2 ; Height in meters.

    db KANGASKHAN 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).

    db SAVANNA DESERT ; Spawning biomes (TYPE as the final argument to apply type-based default tiles).
