    ...
    db 310 ; Dex number.
    db <It stimulates its own muscles with electricity, so it can move quickly. It eases its soreness with electricity, too, so it can recover quickly as well.> ; Dex entry.

    db 40.2 ; Weight in kg.
    db 1.5 ; Height in meters.

    db ELECTRIKE 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).
