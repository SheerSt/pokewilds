    ...
    db 205 ; Dex number.
    db <Forretress conceals itself inside its hardened steel shell. The shell is opened when the Pok�mon is catching prey, but it does so at such a quick pace that the shell's inside cannot be seen.> ; Dex entry.

    db 125.8 ; Weight in kg.
    db 1.2 ; Height in meters.

    db PINECO 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).
