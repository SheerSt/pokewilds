    ...
    db 112 ; Dex number.
    db <It begins walking on its hind legs after evolution. It can punch holes through boulders with its horn.> ; Dex entry.

    db 120.0 ; Weight in kg.
    db 1.9 ; Height in meters.

    db RHYHORN 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).

    db MOUNTAIN ; Spawning biomes (TYPE as the final argument to apply type-based default tiles).
    ...
    db 1 ; Ride.
    db 0 ; Fly.
    db 1 ; Surf (always able on final WATER evolutions).
