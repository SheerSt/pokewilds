    ...
    db 443 ; Dex number.
    db <It lives in caves warmed by geothermal heat. Even so, when the weather gets cold, it will huddle close with others of its kind.> ; Dex entry.

    db 20.5 ; Weight in kg.
    db 0.7 ; Height in meters.

    db GIBLE 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).

    db SAND_FISHING ; Spawning biomes (TYPE as the final argument to apply type-based default tiles).
