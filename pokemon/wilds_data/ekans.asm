    ...
    db 23 ; Dex number.
    db <Ekans curls itself up in a spiral while it rests. Assuming this position allows it to quickly respond to a threat from any direction with a glare from its upraised head.> ; Dex entry.

    db 6.9 ; Weight in kg.
    db 2.0 ; Height in meters.

    db EKANS 100 ; Base form(s) (MON_NAME for Pokemon's name, then chance to hatch; separate groups with a comma).

    db SAVANNA WOODED_LAKE ; Spawning biomes (TYPE as the final argument to apply type-based default tiles).
